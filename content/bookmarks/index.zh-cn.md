---
title: "书签"
description: "我会反复打开的工具、网站和资料入口"
---

这里放一些我会反复打开的链接。它更像一个入口索引：开发、前端、学习、博客、创作工具和日常娱乐网站都会放在这里，但具体工具怎么用、适合什么场景，会拆到对应的专题页里。

## 娱乐视频

- [欧乐影院](https://www.olevod.com/details-1-20130.html) - 经常访问的影视视频站点
- [NO VIP NO AD](https://www.novipnoad.net/) - 经常访问的视频娱乐站点

## 创作工具

- [内容创作者工具箱](/zh-cn/notes/creator-toolkit/) - PixPin、PixelLab、Unsplash 等内容生产、截图、素材和视觉工具的详细说明

## 开发工具

- [VS Code](https://code.visualstudio.com/) - 代码编辑器
- [GitHub](https://github.com/) - 代码托管与协作
- [Vercel](https://vercel.com/) - 前端部署平台
- [Netlify](https://netlify.com/) - 网站托管
- [Phaser](https://github.com/phaserjs/phaser) - 用于网页游戏和互动体验开发的 HTML5 游戏框架，后续可能会用到

## 设计与图标

- [Figma](https://www.figma.com/) - 协作设计工具
- [unDraw](https://undraw.co/) - 开源插图
- [Icones](https://icones.js.org/) - 图标浏览器

## CSS / 前端

- [CSS Tricks](https://css-tricks.com/) - CSS 技巧和教程
- [Can I Use](https://caniuse.com/) - 浏览器兼容性查询
- [Easing Functions](https://easings.net/) - CSS 缓动函数参考

## 开发实用工具

- [Regex 101](https://regex101.com/) - 正则表达式测试
- [Bundle Phobia](https://bundlephobia.com/) - npm 包体积检查
- [Transform](https://transform.tools/) - 代码转换工具

## 学习资源

- [MDN Web Docs](https://developer.mozilla.org/) - Web 技术文档
- [web.dev](https://web.dev/) - 现代 Web 开发指南

## 关注的博客

- [antfu.me](https://antfu.me/) - Anthony Fu 的博客
- [overreacted.io](https://overreacted.io/) - Dan Abramov 的博客
- [joshwcomeau.com](https://www.joshwcomeau.com/) - Josh Comeau 的博客
