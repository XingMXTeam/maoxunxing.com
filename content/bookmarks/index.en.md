---
title: "Bookmarks"
description: "Tools, websites, and resources I open repeatedly"
---

This page is more like a resource index. I keep links for development, frontend work, learning, blogs, and creator tools here. When a tool needs more context, usage notes, or examples, I move the details into a dedicated page.

## Creator Tools

- [Creator toolkit](/notes/creator-toolkit/) - Detailed notes on tools such as PixPin, PixelLab, Unsplash, screenshots, visual assets, and content production

## Development Tools

- [VS Code](https://code.visualstudio.com/) - Code editor
- [GitHub](https://github.com/) - Code hosting and collaboration
- [Vercel](https://vercel.com/) - Frontend deployment platform
- [Netlify](https://netlify.com/) - Web hosting and automation

## Design and Icons

- [Figma](https://www.figma.com/) - Collaborative design tool
- [unDraw](https://undraw.co/) - Open-source illustrations
- [Icones](https://icones.js.org/) - Icon explorer

## CSS / Frontend

- [CSS Tricks](https://css-tricks.com/) - CSS tips and techniques
- [Can I Use](https://caniuse.com/) - Browser compatibility tables
- [Easing Functions](https://easings.net/) - CSS easing reference
- [1-Line Layouts](http://1linelayouts.glitch.me/) - CSS layout patterns

## Dev Utilities

- [Regex 101](https://regex101.com/) - Regular expression tester
- [Bundle Phobia](https://bundlephobia.com/) - npm package size checker
- [Transform](https://transform.tools/) - Code transformers

## Learning

- [MDN Web Docs](https://developer.mozilla.org/) - Web technology reference
- [web.dev](https://web.dev/) - Modern web development guidance

## Blogs I Follow

- [antfu.me](https://antfu.me/) - Anthony Fu's blog
- [overreacted.io](https://overreacted.io/) - Dan Abramov's blog
- [joshwcomeau.com](https://www.joshwcomeau.com/) - Josh Comeau's blog
