---
title: "非法的 Hooks 调用导致按钮无反应问题分析与解决方案"
date: 2025-02-24
---
...
1. **遵守 Hooks 使用规则**：
   - 始终确保 Hooks 只在函数组件或自定义 Hook 中调用。
   - 避免在普通函数、事件回调或生命周期方法中直接调用 Hooks。

2. **开发阶段严格检查**：
   - 配置 ESLint 规则（如 `react-hooks/rules-of-hooks` 和 `react-hooks/exhaustive-deps`），在开发阶段及时发现问题。

3. **线上环境全面监控**：
   - 添加全局错误监听器，捕获未处理的异常。
   - 使用日志上报工具（如 Sentry、Bugsnag）记录和分析线上错误。

4. **处理独立 DOM 树中的错误**：
   - 为独立 DOM 树（如 `Dialog.confirm`）的内容部分封装局部的 `ErrorBoundary`。
   - 对回调函数进行安全封装，捕获其中的错误。

通过以上措施，可以有效避免非法 Hooks 调用导致的问题，提升项目的稳定性和可维护性。