...
- 场景：让 AI 做细节微调，而不是大改页面。
- 适合模型：Cursor、Claude Code、Codex。
- 输出格式：修改建议 + 最小 diff。
...
目标：让另一个 AI 模型可以根据这份 guide 复刻相似风格，但不要逐像素抄袭。
