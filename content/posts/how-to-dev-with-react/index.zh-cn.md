...
> redux的reducer文件名 就是state的挂载对象。 比如说文件名是schedule， 那么reducer内部的所有状态都是挂载在schedule这个对象下
...
> mobx-react的observer对象（一般是react组件）会监听store的数据（@observable的数据变化时会触发）变化，并且会重新渲染
