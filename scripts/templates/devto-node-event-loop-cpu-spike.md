...
tags: node, javascript, performance, webdev
canonical_url: https://maoxunxing.com/node-event-loop-cpu-spike/
---

> This article was originally published on **[maoxunxing.com](https://maoxunxing.com/node-event-loop-cpu-spike/)**. Follow me there for more deep dives on Node.js performance and production debugging.
...
If you've run into similar bottlenecks on the frontend side, my [Frontend Performance Optimization Guide](https://maoxunxing.com/perfomance/) covers the browser-side equivalent of these patterns: how rendering pipelines stall and what metrics to track before users notice.

The `@MessageSubscriber` decorator and dependency injection patterns in this article come from the Midway.js framework. I documented the painful gotchas of [upgrading Midway from v6 to v8](https://maoxunxing.com/nodejs-5/) if you're running a similar stack.

For a broader look at how server-side rendering interacts with event loop pressure, my [React SSR (Streaming Server-Side Rendering)](https://maoxunxing.com/react-3/) article walks through how streaming responses change the concurrency picture compared to traditional SSR.
